-- 
-- This collectd Lua plugin parses vmstat -i output on NetBSD.
-- 
...
local function read_vmstat(stats)
	...
	for l in io.popen("/usr/bin/vmstat -i"):lines() do
		local k, v = l:match("^(.-)%s+(%d+)%s+%d+$")
		if v and k then
			k = "interrupts-" .. k:gsub("%s", "-")
			stats[k] = tonumber(v)
		end
	end
...
local function log_success()
	local msg = "interrupts.lua will report "
