-- 
-- This collectd Lua plugin parses vmstat -f output on NetBSD.
-- 
...
local function read_vmstat(stats)
	...
	for l in io.popen("/usr/bin/vmstat -f"):lines() do
		local v, k = l:match("^(%d+)%s+(.+)$")
		if v and k then
			k = k:gsub("%s", "-")
			stats[k] = tonumber(v)
		end
...
local function log_success()
	local msg = "forks.lua will report "
